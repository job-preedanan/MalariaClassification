function properties = GLCM_Features1(glcmin,pairs)
% 
...
% checked 
properties.autoc = zeros(1,size_glcm_3); % Autocorrelation: [2] 
properties.contr = zeros(1,size_glcm_3); % Contrast: matlab/[1,2]
properties.corrm = zeros(1,size_glcm_3); % Correlation: matlab
properties.corrp = zeros(1,size_glcm_3); % Correlation: [1,2]
properties.cprom = zeros(1,size_glcm_3); % Cluster Prominence: [2]
properties.cshad = zeros(1,size_glcm_3); % Cluster Shade: [2]
properties.dissi = zeros(1,size_glcm_3); % Dissimilarity: [2]
properties.energ = zeros(1,size_glcm_3); % Energy: matlab / [1,2]
properties.entro = zeros(1,size_glcm_3); % Entropy: [2]
properties.homom = zeros(1,size_glcm_3); % Homogeneity: matlab
properties.homop = zeros(1,size_glcm_3); % Homogeneity: [2]
properties.maxpr = zeros(1,size_glcm_3); % Maximum probability: [2]

properties.sosvh = zeros(1,size_glcm_3); % Sum of sqaures: Variance [1]
properties.savgh = zeros(1,size_glcm_3); % Sum average [1]
properties.svarh = zeros(1,size_glcm_3); % Sum variance [1]
properties.senth = zeros(1,size_glcm_3); % Sum entropy [1]
properties.dvarh = zeros(1,size_glcm_3); % Difference variance [4]
%out.dvarh2 = zeros(1,size_glcm_3); % Difference variance [1]
properties.denth = zeros(1,size_glcm_3); % Difference entropy [1]
properties.inf1h = zeros(1,size_glcm_3); % Information measure of correlation1 [1]
properties.inf2h = zeros(1,size_glcm_3); % Informaiton measure of correlation2 [1]
...
properties.indnc = zeros(1,size_glcm_3); % Inverse difference normalized (INN) [3]
properties.idmnc = zeros(1,size_glcm_3); % Inverse difference moment normalized [3]
...
for k = 1:size_glcm_3 % number glcms
    ...
    for i = 1:size_glcm_1
        ...
        for j = 1:size_glcm_2
            ...
            properties.contr(k) = properties.contr(k) + (abs(i - j))^2.*glcm(i,j,k);
            properties.dissi(k) = properties.dissi(k) + (abs(i - j)*glcm(i,j,k));
            properties.energ(k) = properties.energ(k) + (glcm(i,j,k).^2);
            properties.entro(k) = properties.entro(k) - (glcm(i,j,k)*log(glcm(i,j,k) + eps));
            properties.homom(k) = properties.homom(k) + (glcm(i,j,k)/( 1 + abs(i-j) ));
            properties.homop(k) = properties.homop(k) + (glcm(i,j,k)/( 1 + (i - j)^2));
            ...
            properties.sosvh(k) = properties.sosvh(k) + glcm(i,j,k)*((i - glcm_mean(k))^2);
            
            %out.invdc(k) = out.homom(k);
            properties.indnc(k) = properties.indnc(k) + (glcm(i,j,k)/( 1 + (abs(i-j)/size_glcm_1) ));
            properties.idmnc(k) = properties.idmnc(k) + (glcm(i,j,k)/( 1 + ((i - j)/size_glcm_1)^2));
    ...
    properties.maxpr(k) = max(max(glcm(:,:,k)));
end
...
for k = 1:(size_glcm_3)
    ...
    for i = 1:(2*(size_glcm_1)-1)
        properties.savgh(k) = properties.savgh(k) + (i+1)*p_xplusy(i,k);
        % the summation for savgh is for i from 2 to 2*Ng hence (i+1)
        properties.senth(k) = properties.senth(k) - (p_xplusy(i,k)*log(p_xplusy(i,k) + eps));
    end
...
for k = 1:(size_glcm_3)
    ...
    for i = 1:(2*(size_glcm_1)-1)
        properties.svarh(k) = properties.svarh(k) + (((i+1) - properties.senth(k))^2)*p_xplusy(i,k);
        % the summation for savgh is for i from 2 to 2*Ng hence (i+1)
    end
...
% we have for dvarh
    for i = 0:(size_glcm_1-1)
        properties.denth(k) = properties.denth(k) - (p_xminusy(i+1,k)*log(p_xminusy(i+1,k) + eps));
        properties.dvarh(k) = properties.dvarh(k) + (i^2)*p_xminusy(i+1,k);
    end
end

% compute information measure of correlation(1,2) [1]
for k = 1:size_glcm_3
    hxy(k) = properties.entro(k);
    for i = 1:size_glcm_1
...
%             end
    ...
    properties.inf1h(k) = ( hxy(k) - hxy1(k) ) / ( max([hx(k),hy(k)]) );
    properties.inf2h(k) = ( 1 - exp( -2*( hxy2(k) - hxy(k) ) ) )^0.5;
%     eig_Q(k,:)   = eig(Q(:,:,k));
...
for k = 1:size_glcm_3
    for i = 1:size_glcm_1
        for j = 1:size_glcm_2
            ...
            properties.cprom(k) = properties.cprom(k) + (((i + j - u_x(k) - u_y(k))^4)*...
                glcm(i,j,k));
            properties.cshad(k) = properties.cshad(k) + (((i + j - u_x(k) - u_y(k))^3)*...
                glcm(i,j,k));
        end
    end
    ...
    s_x(k) = s_x(k) ^ 0.5;
    s_y(k) = s_y(k) ^ 0.5;
    properties.autoc(k) = corp(k);
    properties.corrp(k) = (corp(k) - u_x(k)*u_y(k))/(s_x(k)*s_y(k));
    properties.corrm(k) = corm(k) / (s_x(k)*s_y(k));
%     % alternate values of u and s
